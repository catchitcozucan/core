    ...
    private String appendCriteriaStatesAndFinalStateAndToggleFormatting() {
        ...
        if (!bpmDescriptors.isEmpty() && bpmDescriptors.get(0) != null) {
            ...
            bpmDescriptors.stream().forEach(s -> { //NOSONAR
                stepsAndStatuses.append(CASE).append(s.getMyStateName()).append(COLON).append(NL).append(CASE).append(s.getStatusUponFailure()).append(COLON).append(NL).append(EXECUTE_STEP).append(s.getStepMethodName()).append(PARENTHESIS_END_SEMI_COLON).append(NL).append(BREAK).append(NL);
            });
